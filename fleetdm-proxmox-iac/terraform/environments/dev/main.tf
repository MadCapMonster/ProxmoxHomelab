...
locals {
  env = "dev"
  vms = {
    app   = { name = "fleet-app-dev",   vm_id = var.vm_base_id + 1, ip = var.fleet_app_ip,   cores = 2, memory = 4096, disk = 40 }
    db    = { name = "fleet-db-dev",    vm_id = var.vm_base_id + 2, ip = var.fleet_db_ip,    cores = 2, memory = 4096, disk = 80 }
    redis = { name = "fleet-redis-dev", vm_id = var.vm_base_id + 3, ip = var.fleet_redis_ip, cores = 1, memory = 2048, disk = 20 }
  }
}
module "fleet_vms" {
  for_each = local.vms
